...
int main()
{
    ...
    char buf2[1024];
    sprintf(buf2, "drawRectLoop 0xF800\r\n");
    write(serial_port, buf2, strlen(buf2));
    ...
    for (;;) {
        for (int i = 0; i < 100; i++) {
            ...
            char buf[1024];
            sprintf(buf, "drawRect %d 10 50 50 0xF800\r\n", i);
            write(serial_port, buf, strlen(buf));
            // 50 ms
            usleep(50);
            sprintf(buf, "drawRect %d 10 50 50 0x0000\r\n", i);
            write(serial_port, buf, strlen(buf));
        }
        printf("reset i\n");
    }
